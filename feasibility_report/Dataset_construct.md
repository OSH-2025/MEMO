...
AIOS的LSFS（Large-Scale Semantic File System，详见调研报告）利用LLM的语义理解能力，从文件内容中提取语义特征并生成相应的嵌入向量，在传统文件系统上外挂向量数据库，为AIOS提供提示词驱动的文件管理服务。但截至2025年3月，LSFS只能处理文本文件（纯文本或二进制），为了实现我们的内存管理宏图，至少要将其拓展至可执行文件（例如.exe、.dll或ELF文件）。
...
- 计算开销大：预处理阶段离线完成，实时操作只调用嵌入结果。新的应用加入后的维护。预计使用API调用的方式。
